...
<style>
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  .stage { flex: 1 1 0; width: 100%; max-width: 480px; display: flex; align-items: center; justify-content: center; perspective: 1200px; padding: 8px 0; min-height: 0; overflow: hidden; }
  .card { position: relative; flex: 0 0 auto; transform-style: preserve-3d; transition: transform 0.55s cubic-bezier(.2,.7,.2,1); overflow: hidden; }
  ...
  .back .artist { font-size: clamp(20px, 5.5vw, 30px); font-weight: 700; margin-top: 0; line-height: 1.15; max-width: 100%; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; word-break: break-word; }
  .back .year { font-size: clamp(72px, 22vw, 140px); font-weight: 900; line-height: 0.95; letter-spacing: -3px; margin: 0.1em 0; color: #000; }
  .back .title { font-size: clamp(16px, 4.6vw, 24px); font-style: italic; font-weight: 500; line-height: 1.25; max-width: 100%; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; word-break: break-word; }
  .back .idx { position: absolute; bottom: 10px; right: 14px; font-size: 11px; opacity: 0.55; font-weight: 600; }
  .back .verify, .back .editBtn, .back .hideBtn { position: absolute; bottom: 8px; font-size: 11px; color: #111; text-decoration: none; opacity: 0.55; padding: 4px 8px; border: 1px solid rgba(0,0,0,0.3); border-radius: 6px; background: rgba(255,255,255,0.25); cursor: pointer; }
  .back .verify { left: 12px; }
  .back .editBtn { left: 76px; }
  .back .hideBtn { left: 156px; }
  .back .verify:active, .back .editBtn:active, .back .hideBtn:active { opacity: 1; }
  ...
  .controls button { padding: 16px; font-size: 15px; font-weight: 600; border-radius: 12px; border: none; background: #2a2a2a; color: #eee; }
  ...
  .status { position: fixed; top: 14px; left: 50%; transform: translateX(-50%); background: #1db954; color: #fff; padding: 6px 14px; border-radius: 999px; font-size: 12px; font-weight: 600; opacity: 0; pointer-events: none; transition: opacity 0.25s; z-index: 20; }
...
<div class="stage">
  ...
  <div class="card" id="card" style="display:none">
    ...
    <div class="face back" id="back">
      ...
      <button class="hideBtn" id="hideBtn" onclick="event.stopPropagation(); hideCurrent()">Ausblenden</button>
    </div>
  </div>
</div>
...
<div class="controls">
  ...
  <button id="flipBtn" disabled>Umdrehen</button>
...
<div class="modal" id="hiddenModal">
  <div class="modal-box" onclick="event.stopPropagation()">
    ...
    <div class="corr-list" id="hiddenList"></div>
    <div class="modal-actions">
...
function effectiveSong(s) {
  ...
  if (c && c.year_correct) {
    return { ...s, year: c.year_correct, _edited: true, _correctedAt: c.correction_date };
  }
...
function sizeCard() {
  ...
  const size = Math.max(160, Math.min(w, h, 480));
  card.style.setProperty('width',      size + 'px', 'important');
  card.style.setProperty('height',     size + 'px', 'important');
  card.style.setProperty('min-width',  size + 'px', 'important');
  card.style.setProperty('min-height', size + 'px', 'important');
  card.style.setProperty('max-width',  size + 'px', 'important');
  card.style.setProperty('max-height', size + 'px', 'important');
}
window.addEventListener('resize', sizeCard);
window.addEventListener('orientationchange', () => setTimeout(sizeCard, 100));
if (typeof ResizeObserver !== 'undefined') {
  // Re-size whenever the stage gets a new layout (after fonts load, etc.)
  new ResizeObserver(sizeCard).observe(document.querySelector('.stage') || document.body);
}
...
async function hideCurrent() {
  ...
  try {
    ...
    updateHiddenCount();
    // Remove from current deck and stay at same position (now shows next song)
    deck.splice(pos, 1);
    if (!deck.length) {
      reshuffle(); // shows empty-state
      return;
    }
...
async function unhide(id) {
  ...
  try {
    ...
    updateHiddenCount();
    // Add to current deck at next position
    const s = songs.find(x => x.id === id);
    if (s) {
      deck.splice(pos + 1, 0, s);
    }
    openHidden(); // refresh list
  } catch (e) {
    hidden[id] = prev;
...
function openHidden() {
  ...
  if (!ids.length) {
    list.innerHTML = '<div class="corr-empty">Keine ausgeblendeten Songs.</div>';
  } else {
    ...
    list.innerHTML = items.map(x => {
      ...
      return `<div class="corr-item" data-id="${x.id}">
        ...
        <div class="year" style="color:#d04040;font-size:14px;">↩</div>
      </div>`;
    }).join('');
